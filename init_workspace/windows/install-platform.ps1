...
function Install-Platform {
    ...
    if ($Arguments) {
        Write-Host "Параметры: $($Arguments -join ' ')"
    }
    ...
    if ([System.IO.Path]::GetExtension($InstallerPath) -ieq ".msi") {
        $startProcessParams.FilePath = "msiexec.exe"
        $startProcessParams.ArgumentList = @("/i", $InstallerPath) + $Arguments
    }

    $process = Start-Process @startProcessParams

    if ($process.ExitCode -ne 0) {
        throw "Установка платформы завершилась с кодом $($process.ExitCode)."
    }
}
